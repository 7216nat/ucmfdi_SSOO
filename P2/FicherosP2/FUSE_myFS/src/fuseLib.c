...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
	...
	/*if (read(myFileSystem.fdVirtualDisk, &buffer, BLOCK_SIZE_BYTES) == -1) {
	    fprintf(stderr,"Error reading blocks in my_read\n");
            return -EIO;
	}*/
...
static int my_unlink(const char *path){ 
    ...
    myFileSystem.directory.numFiles--;
    //update filestruct
    ptrNodeI = myFileSystem.nodes[idxNodeI];
    myFileSystem.directory.files[idxDir].freeFile = true;
    //update Bitmap
    for(int i = 0; i < ptrNodeI->numBlocks; i++){
	myFileSystem.bitMap[ptrNodeI->blocks[i]] = 0;
    }
    //update Superblock
    myFileSystem.superBlock.numOfFreeBlocks += ptrNodeI->numBlocks;
    //update nodeI
    ptrNodeI->freeNode = true; 
    updateNode(&myFileSystem, idxNodeI, ptrNodeI);
